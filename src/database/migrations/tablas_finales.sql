...
CREATE TABLE IF NOT EXISTS salas (
    ...
    lobby_nombre        VARCHAR(255),
    lobby_estado        VARCHAR(30)    NOT NULL DEFAULT 'sin_lobby',
    host_steam_id       VARCHAR(50),
    banda_vacante       VARCHAR(20),
    sorteo_pendiente    BOOLEAN        NOT NULL DEFAULT TRUE,
    match_id            VARCHAR(50),
    match_resultado     JSONB,
    match_duracion      INTEGER,
    match_encontrado_en TIMESTAMP,
    match_consultado_en TIMESTAMP,
    match_finalizado_en TIMESTAMP,
    match_raw_data      JSONB,
...
CREATE TABLE IF NOT EXISTS soporte_chats (
    ...
    admin_id       INTEGER REFERENCES superadmin_usuarios(id) ON DELETE SET NULL,
    estado         VARCHAR(20)  NOT NULL DEFAULT 'activo',
    creado_en      TIMESTAMP    NOT NULL DEFAULT NOW(),
    actualizado_en TIMESTAMP    NOT NULL DEFAULT NOW(),
    CHECK (tipo IN ('usuario', 'admin')),
    CHECK (estado IN ('activo', 'resuelto', 'cerrado'))
);

CREATE TABLE IF NOT EXISTS soporte_mensajes (
    id         SERIAL PRIMARY KEY,
    chat_id    INTEGER   NOT NULL REFERENCES soporte_chats(id) ON DELETE CASCADE,
    usuario_id INTEGER REFERENCES usuarios(id) ON DELETE SET NULL,
    admin_id   INTEGER REFERENCES superadmin_usuarios(id) ON DELETE SET NULL,
    mensaje    TEXT      NOT NULL,
    es_admin   BOOLEAN   NOT NULL DEFAULT FALSE,
...
CREATE INDEX IF NOT EXISTS idx_usuarios_nombre_cambiado_en ON usuarios(nombre_cambiado_en);
...
CREATE INDEX IF NOT EXISTS idx_salas_host_steam_id         ON salas(host_steam_id);
CREATE INDEX IF NOT EXISTS idx_salas_match_id              ON salas(match_id);
...
CREATE INDEX IF NOT EXISTS idx_soporte_chats_usuario       ON soporte_chats(usuario_id);
CREATE INDEX IF NOT EXISTS idx_soporte_chats_admin         ON soporte_chats(admin_id);
CREATE INDEX IF NOT EXISTS idx_soporte_chats_estado        ON soporte_chats(estado);
CREATE INDEX IF NOT EXISTS idx_soporte_chats_actualizado   ON soporte_chats(actualizado_en DESC);
CREATE INDEX IF NOT EXISTS idx_soporte_mensajes_chat       ON soporte_mensajes(chat_id);
CREATE INDEX IF NOT EXISTS idx_soporte_mensajes_creado     ON soporte_mensajes(creado_en);
...
DROP TRIGGER IF EXISTS trg_superadmin_actualizado       ON superadmin_usuarios;
CREATE TRIGGER trg_superadmin_actualizado
    BEFORE UPDATE ON superadmin_usuarios
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_partidas_actualizado         ON partidas;
CREATE TRIGGER trg_partidas_actualizado
    BEFORE UPDATE ON partidas
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_salas_actualizada            ON salas;
CREATE TRIGGER trg_salas_actualizada
    BEFORE UPDATE ON salas
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp_sala();

DROP TRIGGER IF EXISTS trg_partidas_en_vivo_actualizada ON partidas_en_vivo;
CREATE TRIGGER trg_partidas_en_vivo_actualizada
    BEFORE UPDATE ON partidas_en_vivo
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp_sala();

DROP TRIGGER IF EXISTS trg_apuestas_actualizado         ON apuestas;
CREATE TRIGGER trg_apuestas_actualizado
    BEFORE UPDATE ON apuestas
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_salas_resultados_actualizado ON salas_resultados;
CREATE TRIGGER trg_salas_resultados_actualizado
    BEFORE UPDATE ON salas_resultados
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_baneados_actualizado         ON usuarios_baneados_historial;
CREATE TRIGGER trg_baneados_actualizado
    BEFORE UPDATE ON usuarios_baneados_historial
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_guias_actualizado            ON guias;
CREATE TRIGGER trg_guias_actualizado
    BEFORE UPDATE ON guias
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_guias_paginas_actualizado    ON guias_paginas;
CREATE TRIGGER trg_guias_paginas_actualizado
    BEFORE UPDATE ON guias_paginas
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_guias_secciones_actualizado  ON guias_secciones;
CREATE TRIGGER trg_guias_secciones_actualizado
    BEFORE UPDATE ON guias_secciones
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_guias_items_actualizado      ON guias_items;
CREATE TRIGGER trg_guias_items_actualizado
    BEFORE UPDATE ON guias_items
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_bonos_promociones_actualizado ON bonos_promociones;
CREATE TRIGGER trg_bonos_promociones_actualizado
    BEFORE UPDATE ON bonos_promociones
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();

DROP TRIGGER IF EXISTS trg_bonos_usuario_actualizado    ON bonos_usuario;
CREATE TRIGGER trg_bonos_usuario_actualizado
    BEFORE UPDATE ON bonos_usuario
    FOR EACH ROW EXECUTE FUNCTION actualizar_timestamp();
